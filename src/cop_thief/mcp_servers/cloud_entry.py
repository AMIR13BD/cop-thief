...
is deployed twice (one service per role). ``$MCP_APP`` selects which contract:
``match`` (default — the inter-group 8-tool referee, ``match_server``) or
``relay`` (the self-play relay, ``server_app``). Token auth is enforced inside
the tools via ``$MCP_AUTH_TOKEN``, never baked into the image.
...
from .match_server import run_match_server
...
def main() -> None:
    ...
    app = os.environ.get("MCP_APP", "match").lower()
    print(f"Starting {role.value} MCP {app} server on {host}:{port}")
    if app == "relay":
        run_server(role, host, port)
    else:
        run_match_server(role, host, port)
